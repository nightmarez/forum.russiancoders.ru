...
<div class="panel panel-primary" style="margin: 20px 0 20px 0;">
	<div class="panel-heading">
		<h3 class="panel-title">Пользователи OnLine за последнюю неделю</h3>
	</div>
	<script>
		document.title = 'Пользователи OnLine за последнюю неделю';
	</script>
					...
					<?php
						$query = 
							'SELECT `userid`, `login`, `last`, (now() - `last`) AS `online`, MD5(LOWER(TRIM(`mail`))), `state`  
							 FROM `users` WHERE TIME_TO_SEC(TIMEDIFF(NOW(), `last`)) <= 24 * 60 * 60 * 7
